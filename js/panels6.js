    ...
    nextColumn.resizable({
        ...
        resize: function(event, ui) {
            ...
            for (var i=0;i<allSMPanels.length;i++){
                ...
                if (!$(allSMPanels[i]).hasClass("sm-text")) {
                    resizeRect($(allSMPanels[i]).attr("pID"),$(allSMPanels[i]).attr("qID"));
                    //console.log("resized");
                }
            }
...
$('#panels').on('click', 'div div .hide-btn', function() {
    ...
    $("#panel"+panelID+"-survey-area").toggle("slow");
    $("#panel"+panelID+'-question-area').toggle("slow");
});
...
function updateDefaultChart(pID) {
    ...
    for (var i=0; i<currentOptions.length;i++){
        ...
        nextSmallMultiplePanel.css("width",parseInt(nextSmallMultiplePanel.css("width"))-1);
    ...
    if (window.brushSettings[currentSurveyIndex] instanceof Object) {
        brushAllCharts(pID,currentSurveyIndex,window.brushSettings[currentSurveyIndex].qID,window.brushSettings[currentSurveyIndex].response,$("#panel"+pID));
    }
