var stackedBarchart = function() {
	var query_stacked_class = "col-lg-12 col-md-12 col-sm-12 col-xs-12";
	var stacked_q_wording_h = 30;
	...
	function createQueryStacked(qcID) {
		...
		if (qID.length > 1) {
			...
			if (matched == true) {
				$("#query-chart"+qcID+" .panel").append($('<div class="chart-container '+query_stacked_class+'"></div>'));
				drawQueryStacked(qcID, qID, sID);
	...
	function drawQueryStacked(qcID, qID, sID) {
		var currentContainer = $("#query-chart"+qcID+" .chart-container");
		var daID;
		currentContainer.append(newStackedDOM(qcID,sID,qID));
		for (var i=0; i<surveyDistinctAnswer[sID].length; i++) {
			if (utils.equalArrays(surveyDistinctAnswer[sID][i], surveyResponseAnswer[sID][qID[0]])) {
		...
		respList = surveyDistinctAnswer[sID][daID];
		for (var i=0; i<qID.length; i++) {
			addQueryWording(qcID, sID, qID[i], $(questWordingSVG[i]));
			respHistogram = barchart.getHistogramData(respList, qID[i], sID, surveyDataTable[sID][1][qID[i]]);
			//respList = surveyResponseAnswer[sID][qID[0]];
			//console.log(respHistogram)
			//console.log(respList)
			drawStackedBars(qcID,qID[i],$(stackedDIV[i]),respHistogram,respList, sID);
		}

		drawStackedLegend(qcID,daID, sID);
		...
		currentContainer.click(function(evt){
			...
			if(evt.target.tagName == "DIV" | evt.target.tagName == "svg") clearBrushing(sID);
		});
	...
	function addQueryWording(qcID, sID, qID, wordingSVG, direction) {
		...
		.text(function(d){
			return heatmap.fitWordingLength($(this),$(this).parent(),3,direction);
		})
		//.attr("onresize","fitWordingLength($(this),$(this).parent(),3)")
		.append("title").text(function(d){return d});
	}

	function drawStackedBars(qcID, qID, stackedDIV, respHistogram, respList, sID) {
		...
		for (var i=0; i<stackedSVGs.length; i++) {
			...
			.attr("onclick",function(d){
				return 'brushing.brushAllCharts('+sID+',["'+qID+'"],"'+respList[i]+'",0,this);'
			})
	...
	function drawStackedLegend(qcID, daID, sID) {
		...
		for (var i=0; i<colorSVGs.length; i++) {
			$(respSVGs[i]).parent().attr("datavalue",surveyDistinctAnswer[sID][daID][i]).attr("qcID",qcID);
			...
			.data([surveyDistinctAnswer[sID][daID][i]])
			.enter()
			.append("text")
			...
			.text(function(d){return d})
			.text(function(d){
				return heatmap.fitWordingLength($(this),$(this).parent(),3);
			})
			//.attr("onresize","fitWordingLength($(this),$(this).parent(),3)")
			.append("title").text(function(d){return d});
		}
	...
	// Move addQueryWording to another module
	return {
		createQueryStacked: 	createQueryStacked,
		addQueryWording: 		addQueryWording
	}
}();