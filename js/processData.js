var processData = function() {
		...
		for (q in responseAnswerList) {
			...
			if (distinctRespList.length == 0) distinctRespList[0] = responseAnswerList[q];
				...
				for (var i=0; i<distinctRespList.length; i++) {
					if (utils.equalArrays(responseAnswerList[q],distinctRespList[i])) isEqual = false;
				}
				if (isEqual == true) {
		...
		for (var i=0; i<surveyResponseAnswer.length; i++) {
			for (q in surveyResponseAnswer[i]) {
				for (j=0; j<surveyDistinctAnswer[i].length; j++) {
					if (utils.equalArrays(surveyDistinctAnswer[i][j],surveyResponseAnswer[i][q])) {
	...
	return {
		getResponseAnswer: 		getResponseAnswer,
		getDistinctAnswer: 		getDistinctAnswer,
		syncResponseWithDA: 	syncResponseWithDA,
	}
}();