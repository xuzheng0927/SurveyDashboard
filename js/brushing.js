var brushing = function() {
	...
	function brushAllCharts(sID,qID,response,panel,clickedbar,resptype) {
		if ((brushSettings[sID] instanceof Object) == false) brushSettings[sID] = new Object();
		brushSettings[sID] = {"qID":qID,"response":response,"clickedbar":clickedbar,"resptype":resptype};
		// if (brushSettings[sID]["clickedbar"] instanceof Object) clickedbar = brushSettings[sID]["clickedbar"];
		// else brushSettings[sID]["clickedbar"] = clickedbar;
			...
			// }
				...
				.attr("fill",function (d,index) {
					...
					if (responseMatch($(this).attr("index"),qID,response,resptype)) {
						if (utils.equalArrays([currentScatter.attr("qID1"),currentScatter.attr("qID2")],qID)) return "#00bbff";
						else return "#00dddd";
					}
					else return "#333333";
				})
				.attr("stroke",function() {
					if (responseMatch($(this).attr("index"),qID,response,resptype)) {
						if (utils.equalArrays([currentScatter.attr("qID1"),currentScatter.attr("qID2")],qID)) return "cyan";
						else return "black";
					}
					else return "black";
				})
				.attr("stroke-width",function() {
					if (responseMatch($(this).attr("index"),qID,response,resptype)) {
						if (utils.equalArrays([currentScatter.attr("qID1"),currentScatter.attr("qID2")],qID)) return 2;
						else return 1;
					}
					else return 1;
				})
	...
	function responseMatch(respondentID, qID, response, resptype) {
		...
		for (var i=0; i<qID.length; i++) {
			...
			responseChk = surveyDataTable[sID][respondentID][qID[i]];
			if (responseChk == null) return false;
