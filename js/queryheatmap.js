var heatmap = function() {
	var query_heatmap_class = "col-lg-12 col-md-12 col-sm-12 col-xs-12";
	var q1_row_percent = 10;
	var q2_col_percent = 10;
	var q2_resp_percent = 20;
	var cell_height = 40;
	...
	function createQueryHeatmap(qcID){
		...
		if (qID.length == 2) {
			...
			if (!utils.containedInArray("Numeric",[RespType1,RespType2]) & !utils.containedInArray("Ranking Response",[RespType1,RespType2]) 
				& !utils.containedInArray("Open-Ended Response",[RespType1,RespType2])) {
				$("#query-chart"+qcID+" .panel").append($('<div class="chart-container '+query_heatmap_class+'" qID1='+qID[0]+' qID2='+qID[1]+'></div>'));
				drawQueryHeatmap(qcID, qID[0], qID[1], sID);
	...
	function drawQueryHeatmap(qcID, qID1, qID2, sID){
		var currentContainer = $("#query-chart"+qcID+" .chart-container");
		var respNum1 = surveyResponseAnswer[sID][qID1].length;
		var respNum2 = surveyResponseAnswer[sID][qID2].length;
		var daID;
		// var sID = panels.sID;
		currentContainer.css("height",(respNum2+2)*cell_height);
		for (var i=0; i<surveyDistinctAnswer[sID].length; i++) {
			if (utils.equalArrays(surveyDistinctAnswer[sID][i],surveyResponseAnswer[sID][qID1])) {
				daID1 = i;
				//console.log(daID);
			}
			if (utils.equalArrays(surveyDistinctAnswer[sID][i],surveyResponseAnswer[sID][qID2])) {
				daID2 = i;
			}
		}
		...
		addQ1Wording(qcID,sID,qID1);
		addQ2Wording(qcID,sID,qID2);
		addQ1Resp(qcID,sID,qID1,daID1);
		addQ2Resp(qcID,sID,qID2,qID1,daID2);
		addHeatmapCells(qcID,sID,qID1,qID2);
		...
		if (daID1 == daID2) {
			$("#query-chart"+qcID).find(".another-btn").show();
			$("#query-chart"+qcID).find(".another-btn").click(function(){
				stackedBarchart.createQueryStacked(qcID);
				if (panels.getBrushSettings()[sID] instanceof Object) {
	        		brushing.brushAllCharts(sID,panels.brushSettings[sID].qID,panels.brushSettings[sID].response,$("#query-area"),panels.brushSettings[sID].clickedbar,panels.brushSettings[sID].resptype);
	    		}
			})
		}
		else $("#query-chart"+qcID).find(".another-btn").hide();
	}

	function newQueryHeatMapDOM (qcID, sID, qID1, qID2, daID1, daID2) {
		var currentContainer = $("#query-chart"+qcID+" .chart-container");
		var respNum1 = surveyResponseAnswer[sID][currentContainer.attr("qID1")].length;
		var respNum2 = surveyResponseAnswer[sID][currentContainer.attr("qID2")].length;
		var currentConWidth = parseInt(currentContainer.css("width"));
		// var daID;
	...
	function addQ1Resp (qcID, sID, qID1, daID) {
		...
		$("#qc"+qcID+"-qID1"+qID1+"-row").sortable({
			...
			stop: function(event,ui) {
				...
				barchart.syncRespOrder(ui.item);
		...
		for (var i1 = 0; i1 < surveyResponseAnswer[sID][qID1].length; i1++) {
			...
			for (var i2 = 0; i2 < surveyResponseAnswer[sID][qID2].length; i2++) {
				...
				for (var j = 2; j < surveyDataTable[sID].length; j++) {
					...
					if ((surveyDataTable[sID][j][qID1] instanceof Array ? utils.containedInArray(surveyResponseAnswer[sID][qID1][i1],surveyDataTable[sID][j][qID1])
						: surveyDataTable[sID][j][qID1] == surveyResponseAnswer[sID][qID1][i1]) &
						(surveyDataTable[sID][j][qID2] instanceof Array ? utils.containedInArray(surveyResponseAnswer[sID][qID2][i2],surveyDataTable[sID][j][qID2])
	...
	// Move fitWordingLength to another module
	return  {
		createQueryHeatmap: 	createQueryHeatmap,
		fitWordingLength: 		fitWordingLength
	}
}();